...
static void update_export_data_v4(void)
{
	...
	for (i = 0; i < app.n_rx_cores; i++) {
		table = app.flow_table_v4[i];

		for (j = 0; j < FLOW_TABLE_SIZE; j++) {
			entry = &table->entries[j];
			exp_entry = &exp_table_v4->entries[i * FLOW_TABLE_SIZE + j];
			...
			if (entry->start_tsc != 0) {
				...
				if (pps == 0) {
					...
					if (curr_tsc - entry->last_tsc > threshold_tsc) {
						ret = remove_flow_entry(table, &entry->key);
				...
				exp_idx_v4[num++] = i * FLOW_TABLE_SIZE + j;
...
static void update_export_data_v6(void)
{
	...
	for (i = 0; i < app.n_rx_cores; i++) {
		table = app.flow_table_v6[i];

		for (j = 0; j < FLOW_TABLE_SIZE; j++) {
			entry = &table->entries[j];
			exp_entry = &exp_table_v6->entries[i * FLOW_TABLE_SIZE + j];
			...
			if (entry->start_tsc != 0) {
				...
				exp_idx_v6[num++] = i * FLOW_TABLE_SIZE + j;
				exp_entry->prev_pkt_cnt = pkt_cnt;
				exp_entry->prev_byte_cnt = byte_cnt;
			}
		}
...
void app_main_loop_monitor(void)
{
	...
	const uint32_t max_entries = FLOW_TABLE_SIZE * app.n_rx_cores;
	...
	exp_table_v4 = (struct export_table_v4 *) rte_zmalloc("exp_table_v4",
			sizeof(struct export_table_v4) + sizeof(struct flowexp_data_v4) * max_entries, 0);
	...
	exp_table_v6 = (struct export_table_v6 *) rte_zmalloc("exp_table_v6",
			sizeof(struct export_table_v6) + sizeof(struct flowexp_data_v6) * max_entries, 0);
	if (exp_table_v6 == NULL)
		rte_exit(EXIT_FAILURE, "Cannot allocate exp_table_v6\n");

	exp_idx_v4 = (uint32_t *) rte_zmalloc("exp_idx_v4", sizeof(uint32_t) * max_entries, 0);
	if (exp_idx_v4 == NULL)
		rte_exit(EXIT_FAILURE, "Cannot allocate exp_idx_v4\n");

	exp_idx_v6 = (uint32_t *) rte_zmalloc("exp_idx_v6", sizeof(uint32_t) * max_entries, 0);
	if (exp_idx_v6 == NULL)
